...
void main() {
  // Tidak perlu WidgetsFlutterBinding.ensureInitialized() di sini —
  // berbeda dengan P4 (sqflite), package:http tidak pakai platform channel.
  runApp(const MyApp());
}
...
class Catatan {
  final int? id; // null = belum diinsert (belum punya id dari server)
  final String judul;
  final String isi;
  ...
  /// Dart object → JSON (Map) untuk dikirim sebagai body request.
  Map<String, dynamic> toJson() => {
        if (id != null) 'id': id, // jangan kirim id kalau masih null
        'judul': judul,
        'isi': isi,
        'kategori': kategori,
        // ISO-8601 string, bukan int epoch seperti di P4 — JSON tidak punya tipe Date.
        'dibuat_pada': dibuatPada.toUtc().toIso8601String(),
      };

  /// JSON (Map) hasil response → Dart object.
...
class _HomePageState extends State<HomePage> {
  ...
  /// Satu-satunya perubahan dari P4: DbHelper.instance -> ApiClient.instance
  void _muatUlang() {
    setState(() {
  ...
  /// Buka form CREATE atau EDIT, lalu reload dari server setelah kembali.
...
// ─────────────────────────────────────────
// HALAMAN 2: CatatanFormPage (CREATE + EDIT)
// ─────────────────────────────────────────
...
class _CatatanFormPageState extends State<CatatanFormPage> {
  ...
  Future<void> _simpan() async {
    ...
    try {
      ...
      // Beda dengan P4: tangkap ApiException secara spesifik supaya
      // pesan dari server (e.message) yang tampil, bukan stacktrace generic.
      if (!mounted) return;
...
String _formatTanggal(DateTime dt) {
  ...
  // dt sudah dalam UTC dari server (toIso8601String) — konversi ke lokal untuk tampilan.
